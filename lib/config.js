...
const config = {
  ...
  // Hard ceiling on one scan. Retries mean a bad network could otherwise stretch a scan
  // far past the poll interval, blocking every later check behind it. Fractional values
  // are allowed (tests use seconds); zero or nonsense falls back rather than disabling
  // scanning outright, which is what an integer parse of "0.05" would have done.
  maxScanMinutes: (() => {
    const v = parseFloat(process.env.MAX_SCAN_MINUTES);
    return Number.isFinite(v) && v > 0 ? v : 6;
  })(),

  // Notifications. SMTP_USER is only a sane fallback recipient when it's an actual
  // address — Resend's SMTP username is the literal string "resend".
  notifyTo: process.env.NOTIFY_TO || (/@/.test(process.env.SMTP_USER || '') ? process.env.SMTP_USER : ''),
